...
Everything here assumes the animation belongs. Whether it belongs at all — what it costs the user at its trigger frequency, and whether it should be shortened, made one-sided, or deleted — is a separate question, and the answer defaults to no.

Honoring `prefers-reduced-motion` is covered by the Accessibility principles in `SKILL.md`; apply it to every animation here.
